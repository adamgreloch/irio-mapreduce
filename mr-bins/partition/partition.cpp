...
void partitionFunction(int R, const std::string& inputFilePath, const std::string& outputDirectory) {
    ...
    for (int i = 0; i < R; ++i) {
        ...
        std::string outputPath = outputDirectory + "/partition" + ss.str() + ".txt";
...
int main(int argc, char* argv[]) {
    if (argc != 7) {
        std::cerr << "Usage: " << argv[0] << " -R <R-param> -i <input_file_path> -o <output_directory>" << std::endl;
        return 1;
    }
